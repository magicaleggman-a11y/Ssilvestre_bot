...
function loadState() {
  ...
  return { openPositions: {}, dailyTrades: [], totalSimPnl: 0, simAmount: DEFAULT_SIM_AMOUNT };
}
...
bot.onText(/\/setsol(?:\s+([\d.]+))?/, (msg, match) => {
  ...
  if (!value || value <= 0) {
    bot.sendMessage(msg.chat.id, 'Usalo así: `/setsol 2` (para simular 2 SOL por trade)', {
      parse_mode: 'Markdown',
    });
    return;
  }
