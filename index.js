...
async function updateHeliusWebhookAddresses(newAddressList) {
  ...
  // 2. La actualizamos con la nueva lista de wallets, dejando todo lo demás igual
  const updated = { ...current, accountAddresses: newAddressList };
  ...
  if (!putRes.ok) throw new Error(`Helius rechazó la actualización (status ${putRes.status})`);
}
